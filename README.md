...
| 参数                    | 值 | 说明                                                          |
|-----------------------|---|-------------------------------------------------------------|
| flight_mode           | 0 | 数据请求延时（开局起飞时视角疯狂上抬请改这个为1,单位是秒，控制飞机上下的速度）                    |
| decelerate            | 0 | 离战区多少公里开始开启减速板（0,代表不开启减速板。这个距离和飞机的飞行高度和速度有关，自己根据飞机调整，单位是公里） |
| reorienting_direction | 0 | 方向调整延时（控制飞机左右调整的速度，单位为秒）                                    |
