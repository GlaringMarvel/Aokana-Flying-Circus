...
# 读取文件设置
_, decelerate_distance, _ = OpenFile.read_values()
...
# 航向控制
def heading_control(IAS, map_size, time_flag, num):
    if IAS < 500:
        flag = 0  # 不适合调整航向
        return flag
    elif time_flag == 2:
...
# 飞向对方机场
def enemy_airfield(map_size, speed_brake):
    # 获得机场坐标和玩家坐标
    ...
    print(f"目标航向{heading},飞机航向{compass},航向修正{delta}")
    if distance < 5 and speed_brake == 0:
        flag = 1            # 开启减速板
        return flag
    # 看小键盘，4左转，6右转
    if -10 > delta >= -180:
